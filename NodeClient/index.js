...
client.connect(5882, "127.0.0.1", () => {
  ...
  // Define header
  const body = Buffer.from("Hello, im the King", "utf-8");
  ...
  setInterval(() => {
    const body = Buffer.from("Hello Agian", "utf-8");
    const bodyLength = body.length;
    const buffer = Buffer.alloc(HEADER_SIZE);
    const version = 1;
    const msgType = 42; // Example: Message type as a u16 (2 bytes)
  ...
  //   const body = Buffer.from("Fuck you Index2", "utf-8");
  //   const bodyLength = body.length;
  //   const buffer = Buffer.alloc(HEADER_SIZE);
  //   const version = 1;
  //   const msgType = 42; // Example: Message type as a u16 (2 bytes)
  ...
  //   buffer.writeUInt16LE(msgType, 6); // Message type (2 bytes)
  //   client.write(Buffer.concat([buffer, body]));
  // }, 5000); // Sleeps for 2 seconds (2000ms)
});

// Handle server response
client.on("data", (data) => {
  console.log("Index2:", data.toString());
});
